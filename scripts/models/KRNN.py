...
class CNNEncoderBase(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # x shape: [batch_size, total_features] where total_features = seq_len * input_dim
        batch_size = x.shape[0]

        # Calculate sequence length based on total features and input dimension
        total_features = x.shape[1]
        if total_features % self.input_dim != 0:
            raise ValueError(f"Total features {total_features} not divisible by input_dim {self.input_dim}")

        seq_len = total_features // self.input_dim
        ...
        # Apply convolution: [batch_size, output_dim, seq_len]
        y = self.conv(x)

        # Permute back: [batch_size, seq_len, output_dim]
        y = y.permute(0, 2, 1)
...
class KRNNEncoderBase(nn.Module):
    def __init__(self, input_dim, output_dim, dup_num, rnn_layers, dropout, device):
        ...
        for _ in range(dup_num):
            self.rnn_modules.append(
                nn.GRU(input_dim, output_dim, num_layers=self.rnn_layers, dropout=dropout if rnn_layers > 1 else 0))
    ...
    def forward(self, x):
        ...
        ----------
        x : torch.Tensor
            Input data [batch_size, seq_len, input_dim]

        Returns
        -------
        torch.Tensor
            Updated representations [batch_size, seq_len, output_dim]
        """
        # Input shape: [batch_size, seq_len, input_dim]
        # For GRU, we need: [seq_len, batch_size, input_dim]
        ...
        # Stack and average the outputs from parallel RNNs
        # [seq_len, batch_size, output_dim, dup_num]
        hids = torch.stack(hids, dim=-1)
        # Average across duplicates: [seq_len, batch_size, output_dim]
        hids = hids.mean(dim=-1)
        # Permute back: [batch_size, seq_len, output_dim]
...
class CNNKRNNEncoder(nn.Module):
    def __init__(
            self, cnn_input_dim, cnn_output_dim, cnn_kernel_size, rnn_output_dim, rnn_dup_num, rnn_layers, dropout,
            device
    ):
...
class KRNNModel(nn.Module):
    ...
    def forward(self, x):
        # x: [batch_size, total_features] -> encoder processes this
        encode = self.encoder(x)  # [batch_size, seq_len, rnn_dim]
...
class KRNN(Model):
    ...
    def fit(
        ...
        self.logger.info(f"Validation data shape: {x_valid.shape}, {y_valid.shape}")
        self.logger.info(f"Feature dimension per time step: {self.fea_dim}")
